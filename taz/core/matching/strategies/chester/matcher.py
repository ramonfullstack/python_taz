...
class ProductMatcher(Matcher):
    ...
    def _find_direct_relatives_including_self(
        self,
        variation: Dict
    ) -> Optional[List]:
        sku = variation['sku']
        ...
        if not product.get('matching_uuid'):
            return [product]

        group_products = list(
            self.raw_products.find(
                {
                    'matching_uuid': product['matching_uuid'],
                    'matching_strategy': CHESTER_STRATEGY
                }
            )
        )
    ...
    def _group_variations(self, variations: List) -> Dict:
        ...
        def build_key(variation: Dict) -> str:
